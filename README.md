...
3. Change `DTSTART` / `DTEND` (format `YYYYMMDDTHHMMSS`), and `LOCATION` if necessary
4. Increment the `SEQUENCE:` so that calendar apps treat the edit as authoritative
5. Commit the change
...
The `.ics` files are the only source of truth: `index.html` doesn't store any fixture data itself. Each team's "show fixtures" dropdown fetches and parses that team's `.ics` file in the browser, so any edit here is reflected on the site immediately — no rebuild step.

## Notes

- Times come from each club's home night; every match is blocked out for 3 hours
- Venue addresses are filled in for every venue
